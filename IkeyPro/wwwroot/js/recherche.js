...
document.getElementById("cat").addEventListener("change", Editeur_List);
...
function Cat_List() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < liste.length; i++) {
                s += "<option value=" + liste[i].idCategorie + ">" + liste[i].categorie + "</option>";
            }
    ...
    xhr.open("GET", "Ajax?action=RechCat", true);
    xhr.send();
}
...
function Editeur_List() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < liste1.length; i++) {
                s += "<option value=" + liste1[i].id_Editeur + ">" + liste1[i].Editeur + "</option>";
    ...
    xhr.open("GET", "Ajax?action=RechEditeur&cat=" + val, true);
    xhr.send();
}


function duree_List() {
    var val1 = document.getElementById("cat").value;
    ...
    xhr.open("GET", "Ajax?action=RechEdition&cat=" + val1 + "&editeur=" + val2, true);
    xhr.send();
}
...
function chercher() {
    ...
    val1 = document.getElementById("cat").value;
